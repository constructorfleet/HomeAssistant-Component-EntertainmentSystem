...
from homeassistant.components.media_player import (ENTITY_ID_FORMAT,
                                                   MediaPlayerEntity,
                                                   PLATFORM_SCHEMA)
...
_entity_or_template = vol.Or([cv.entity_id, cv.template])

ENTERTAINMENT_SYSTEM_SCHEMA = vol.Schema({
        vol.Required(CONF_NAME): cv.string,
        vol.Required(CONF_POWER_STATE): _entity_or_template,
        vol.Required(CONF_MEDIA_STATE): _entity_or_template,
        vol.Required(CONF_AVAILABILITY): _entity_or_template,
        vol.Required(CONF_REMOTE): _entity_or_template,
        vol.Required(CONF_SPEAKER): vol.All(cv.ensure_list, [_entity_or_template]),
        })
#
...
#                 vol.Optional(CONF_SOUND_MODES, default={}): {
#                         cv.string: cv.SCRIPT_SCHEMA
#                         },
...
class EntertainmentSystem(TemplateEntity, MediaPlayerEntity):
